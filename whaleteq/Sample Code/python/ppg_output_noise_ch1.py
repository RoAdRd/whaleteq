from ctypes import *
from aecg100 import *
import sys
import time

device = AECG100(get_lib_path())
if device.connect(-1, 5000) == False:
    print('Error: device is not connected')
    device.free()
    sys.exit()

print('device is connected... ({} / {})'.format(
    device.get_serial_number(),
    device.get_ppg_serial_number()))
sys.stdout.flush()

time.sleep(5)

print('output PPG (white noise) ...')
sys.stdout.flush()

ppg_waveform = device.get_default_ppg_ch1_waveform()
ppg_waveform.NoiseAmplitude = 2
ppg_waveform.NoiseFrequency = PPGNoiseFrequency.WhiteNoise.value

device.output_ppg(PPGChannel.Channel1.value, pointer(
    ppg_waveform), OutputSignalCallback(0))

time.sleep(10)

print('output PPG (50Hz noise) ...')
sys.stdout.flush()

ppg_waveform = device.get_default_ppg_ch1_waveform()
ppg_waveform.NoiseAmplitude = 2
ppg_waveform.NoiseFrequency = PPGNoiseFrequency._50Hz.value

device.output_ppg(PPGChannel.Channel1.value, pointer(
    ppg_waveform), OutputSignalCallback(0))

time.sleep(10)


device.stop_output()
device.free()
